...
- non-view adapter functions can only be called as part of the multicall, to ensure that there is always a collateral check afterwards, regardless of adapter implementation specifics. This is ensured by adding `creditFacadeOnly` modifier from the AbstractAdapter base class;
...
To make state-changing functions secure, they must adhere to the following guidelines:

- they are called as part of the multicall (must have `creditFacadeOnly`) and operate on the account on which it is executed;
...
In some cases, gateways are required to implement delayed withdrawals for Credit Accounts, if the delayed withdrawal implementation on the target contract lack some features, or, on the contrary have some features that potentially make withdrawals unsafe as-is. Typical cases where withdrawal gateways are needed:

- If the target contract allows to claim a mature withdrawal on behalf of another user without pre-approval. In this case, the funds will be received by the credit account, but since the balance change happened outside a Gearbox interaction, quotas will not be updated. As the withdrawal phantom token (see below) will likely be spent and the quota for the output asset will not be set, the user's account will immediately become liquidatable;

- If the target contract does not allow to index the total withdrawal amount by the account only and additional data is needed. This is usually the case when withdrawals are discrete and non-fungible. The additional data is then recorded in the gateway on initiating a withdrawal, allowing to query the withdrawn amount only by the Credit Account's address;

- If the target contract does not allow to withdraw a specific amount of an asset (in many cases the protocol only allows withdrawing the full requested amount). This is needed for partial liquidations, and the gateway can be used to add this functionality.

In all of these cases, the gateway will take the withdrawable asset from the Credit Account, initiate a withdrawal to itself, and record the withdrawal details in an internal map. Once the withdrawal matures, the Credit Account can interact with the gateway (via an adapter) to claim a withdrawal - the gateway will then claim from the target protocol and send the output to the user. Usually, this means that the withdrawal phantom token (see below) will use the gateway's data to determine its balance.

As the gateway basically owns the withdrawals of Credit Accounts while they mature, it's critical to ensure that it is impossible for a user to claim more than they are owed by manipulating gateway's math or via other means.
